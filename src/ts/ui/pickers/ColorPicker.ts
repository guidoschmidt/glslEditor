...
export class ColorPicker extends Picker {
  ...
  init() {
    ...
    this.dom.luminanceBar = this.el.querySelector(
  ...
  draw() {
    ...
    this.dom.hsvBarBGLayer.style.backgroundColor =
      "rgb(" +
      color.hueRGB.r +
      "," +
      color.hueRGB.g +
    ...
    if (this.dom.hsvLeftCursor) {
      this.dom.hsvLeftCursor.style.top = this.dom.hsvRightCursor.style.top =
        (1 - color.hsv.v / 255) * colorDiscRadius * 2 + "px";
    }
  }
  ...
  presentModal(x, y) {
    ...
    this.onHsvDownHandler = addEvent(
      this.dom.hsvMap,
      "mousedown",
      this.onHsvDown,
      this
  ...
  onHsvDown(event) {
    ...
    this.onHsvMoveHandler = addEvent(
      this.el,
      "mousemove",
      this.onHsvMove,
      this
  ...
  onHsvMove(event) {
    ...
    let r, x, y, h, s;
    if (
      event.target === this.dom.hsvMapCover &&
      currentTarget === this.dom.hsvMap
    ) {
      // the circle
      (r = currentTargetHeight / 2),
        ...
        (y = event.offsetY - r),
        (h =
          (360 - ((Math.atan2(y, x) * 180) / Math.PI + (y < 0 ? 360 : 0))) /
          360),
        (s = Math.sqrt(x * x + y * y) / r);
      this.value.set({ h, s }, "hsv");
    } else if (
      event.target === this.dom.hsvBarCursors &&
      currentTarget === this.dom.hsvBarCursors
    ) {
      // the luminanceBar
