import { FloatPicker, Vec2Picker, ColorPicker, Vec3Picker } from "./pickers";
      ...
      if (match) {
        ...
        if (match.type === "color") {
          ...
          this.activeModal.on("linkButton", (color) => {
            this.activeModal = new Vec3Picker(
              color.getString("vec"),
              this.properties
            );
            this.activeModal.showAt(this.main.editor);
        ...
        this.main.options.tooltips &&
        (token.type === "builtin" || token.type === "variable-3")
      ) {
        this.main.visualDebugger.clean(event);
        let html =
          '<p>Learn more about: <a href="https://thebookofshaders.com/glossary/?search=' +
          token.string +
          '" target="_blank">' +
          token.string +
          "</a></p>";
