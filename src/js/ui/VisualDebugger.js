import {
  ...
  isLineAfterMain,
  getVariableType,
  getShaderForTypeVarInLine,
  getResultRange,
  getDeltaSum,
  getHits,
  getMedian,
} from "../tools/debugging";
import { unfocusLine, focusLine, unfocusAll, focusAll } from "../core/Editor";
...
export default class VisualDebugger {
  ...
  testLine(nLine) {
    let cm = mainGE.editor;
    let visualDebugger = mainGE.visualDebugger;
    ...
    if (nLine >= cm.getDoc().size) {
      ...
      for (let i in results) {
        ...
        let markerHTML = "<div>" + results[i].ms.toFixed(2);
    ...
    if (isLineAfterMain(cm, nLine)) {
      ...
      let shader = mainGE.shader.canvas;
  ...
  onTest(target) {
    ...
    if (target.wasValid) {
      ...
      if (visualDebugger.testingSamples.length < N_SAMPLES - 1) {
        ...
        visualDebugger.testingResults.push({
          line: visualDebugger.testingLine,
          ms: target.timeElapsedMs,
          delta: delta,
        });
  ...
  debug(variable, nLine) {
    ...
    if (isLineAfterMain(this.main.editor, nLine)) {
      ...
      if (type !== "none") {
        ...
        this.main.shader.canvas.load(
          getShaderForTypeVarInLine(this.main.editor, type, variable, nLine)
        );
