...
export function getVariableType(cm, sVariable) {
  ...
  let uniformRE = new RegExp(
    "\\s*uniform\\s+(float|vec2|vec3|vec4)\\s+" + sVariable + "\\s*;"
  );
  let voidRE = new RegExp("void main\\s*\\(\\s*[void]*\\)", "i");
  let voidIN = false;
  let constructRE = new RegExp(
    "(float|vec\\d)\\s+(" + sVariable + ")\\s*[;]?",
    "i"
  );
...
export function getResultRange(testResults) {
  ...
  return {
    min: { line: minLine, ms: minMS },
    max: { line: maxLine, ms: maxMS },
  };
}
